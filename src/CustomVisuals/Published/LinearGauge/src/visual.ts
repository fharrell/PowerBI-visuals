module powerbi.extensibility.visual {
    ...
    export class LinearGauge implements IVisual {
        ...
        private animation1($this: this, options: VisualUpdateOptions, data: ILinearGauge, actualFlag: boolean) {
            if (categoryFlag === 1) {
                    ...
                    .on("click", function (d: any, i: number): void {
                        if (d3.select(this).classed("selected")) {
                            ...
                            if (tempSelectionId !== null) {
                                tempSelectionId = null;
                                $this.roscCallFlag = false; $this.update(options);
                            }
                            ...
                            $this.selectionManager.select(d.identity).then((ids: ISelectionId[]) => {
                                ...
                                $this.update(options); d3.select(this).classed("selected", true);
                ...
                $(".navArrow").click((): void => {
                    ...
                    legendItem_selectAll.on("click", function (d: any, i: number): void {
                        if (d3.select(this).classed("selected")) {
                            $this.selectionManager.clear();
                            if (tempSelectionId !== null) { tempSelectionId = null; $this.roscCallFlag = false; $this.update(options); }
                            legendItem_selectAll.classed("selected", false);
                            legendItem_selectAll.style("opacity", 1);
                        } else {
                            $this.selectionManager.select(d.identity).then((ids: ISelectionId[]) => {
        ...
        public converter(dataView: DataView, options: VisualUpdateOptions): ILinearGauge {
            let $this: this = this;
            let clearFlag: boolean = true; tooltipFLag = 0;
            let data: ILinearGauge = LinearGauge.GETDEFAULTDATA();
            linearDataPoint = []; $this.getSettings(this.dataView.metadata.objects);
            dataPoints = [];
            ...
            for (let i: number = 0; i < values.length; i++) {
                let col: DataViewMetadataColumn = dataView.categorical.values[i].source; let value: number = null;
                let pos: number = 0;
                if (col.roles[`Y`]) {
                    ...
                    for (let j: number = 0; j < linearDataPoint.length; j++) {
                        if (tempSelectionId !== null && linearDataPoint[j].selectionId[`key`] === tempSelectionId[`key`]) {
                            value = <number>value + linearDataPoint[j].actualValue; selectedKey = linearDataPoint[j].key;
                            if (individualFlag === 0) { actualValue[pos] = value; pos++; }
                        } else if (tempSelectionId === null) { value = <number>value + linearDataPoint[j].actualValue; if (individualFlag === 0) { actualValue[j] = value; } }
                    }
                    data.actual = value; actualFlag = true; data.actualExists = true; data.actualColName = col.displayName;
                }
                if (col.roles[`MinValue`]) {
                    ...
                    for (const j of linearDataPoint) {
                        ...
                        if (tempSelectionId !== null && j.selectionId[`key`] === tempSelectionId[`key`]) { value = <number>j.minValue; } else if (tempSelectionId === null && value > linearValue) {
                            value = <number>j.minValue;
                        }
                    }
                    data.min = value; data.minColName = col.displayName; data.minFormat = col.format;
                }
                if (col.roles[`MaxValue`]) {
                    data.maxFlag = true; value = null;
                    for (const j of linearDataPoint) {
                        if (tempSelectionId !== null && j.selectionId[`key`] === tempSelectionId[`key`]) { value = <number>j.maxValue; } else if (tempSelectionId === null) { value = <number>value + j.maxValue; }
                    }
                    data.max = value; data.maxColName = col.displayName;
                    data.maxFormat = col.format;
                }
                if (col.roles[`TargetValue`]) {
                    data.targetSet = true; data.targetFormat = col.format; value = null;
                    for (const j of linearDataPoint) {
                    ...
                    data.target = value; data.targetExists = true;
                ...
                if (col.roles[`QualitativeState1Value`]) {
                    ...
                    data.trendValueOne = value; data.trend1Format = col.format; data.trend1Exists = true;
                ...
                if (col.roles[`QualitativeState2Value`]) {
                    ...
                    for (const j of linearDataPoint) {
                        ...
                        } else if (tempSelectionId === null) { value = <number>value + j.trendTwo; }
                    }
                    data.trendValueTwo = value; data.trend2Format = col.format; data.trend2Exists = true;
                ...
                if (col.roles[`BestValue`]) {
                    ...
                    data.best = value; data.bestFormat = col.format;
                    data.bestSet = true;
                    data.bestColName = col.displayName;
                }
            }
            this.animation1($this, options, data, actualFlag);
            return data;
        }
        ...
        public getSettings(objects: DataViewObjects): void {
            ...
            if (this.settings.fillOption === `value`) {
                this.colorsGlobal.push(this.settings.Zone4); this.colorsGlobal.push(this.settings.Zone3);
                this.colorsGlobal.push(this.settings.Zone2); this.colorsGlobal.push(this.settings.Zone1);
            }
            else {
                this.colorsGlobal.push(this.settings.area3); this.colorsGlobal.push(this.settings.area2); this.colorsGlobal.push(this.settings.area1);
            }
        }
        ...
        private showscale(translateVal: string, axisFunction: d3.svg.Axis, className: string, height: number, $this: this, axisFormatter: IValueFormatter, options: VisualUpdateOptions, halfMaxFormattedDataWidth: number, viewport: IViewport) {
        ...
        private showtrend(trend1Val: string, options: VisualUpdateOptions, availablewidth: number, upArrow: string, percentageFontTrend: number,
            trend2Val: string, className: string, translateVal: string, axisFunction: d3.svg.Axis, height: number, $this: this, axisFormatter: IValueFormatter, halfMaxFormattedDataWidth: number, viewport: IViewport) {
            if (this.settings.showTrend) {
                let updatedText1: string; let updatedText2: string;
                const arrowColorI1: string = this.settings.Indicator1; const arrowColorI2: string = this.settings.Indicator2;
                ...
                if (this.data.trend2Exists) {
                    ...
                    if (this.settings.Orientation === "Horizontal") { updatedText2 = textMeasurementService.getTailoredTextOrDefault(textProps2, (options.viewport.width / 2.1) - 20); } else {
                    ...
                    if (this.data.trendValueTwo < 0) { $(".trendvalue2arrow").css({ display: "inline-block", transform: "rotate(90deg)" }); }
                } else { this.trendValue2.style(`display`, `none`); }
        ...
        private fill(gradient: d3.Selection<SVGElement>, range: d3.selection.Update<number>, offSetValue: number[]) {
            ...
            range.style("fill", (): string => {
                ...
                if (this.settings.colorOption === `solidcolor`) {
                    gradient.append("stop").attr("offset", "0%")
                        .attr("stop-color",
                            context.getDarkShade(context.colorsGlobal[4], 0.5)).attr("stop-opacity", 1);
                    gradient.append("stop")
                        .attr("offset", `${offSetValue[4]}%`)
                        .attr("stop-color", context.colorsGlobal[4])
                        .attr("stop-opacity", 1);
                    gradient.append("stop").attr("offset", `${offSetValue[4]}%`)
                        .attr("stop-color",
                            context.getDarkShade(context.colorsGlobal[3], 0.5)).attr("stop-opacity", 1);
                    gradient.append("stop")
                        .attr("offset", `${offSetValue[3]}%`)
                        .attr("stop-color", context.colorsGlobal[3])
                        .attr("stop-opacity", 1);
                    gradient.append("stop").attr("offset", `${offSetValue[3]}%`)
                        .attr("stop-color",
                            context.getDarkShade(context.colorsGlobal[2], 0.5)).attr("stop-opacity", 1);
                    gradient.append("stop")
                        .attr("offset", `${offSetValue[2]}%`)
                        .attr("stop-color", context.colorsGlobal[2])
                        .attr("stop-opacity", 1);
                    gradient.append("stop").attr("offset", `${offSetValue[2]}%`)
                        .attr("stop-color",
                            context.getDarkShade(context.colorsGlobal[1], 0.5)).attr("stop-opacity", 1);
                    gradient.append("stop")
                        .attr("offset", `${offSetValue[1]}%`)
                        .attr("stop-color", context.colorsGlobal[1])
                        .attr("stop-opacity", 1);
        ...
        private colorfill(range: d3.selection.Update<number>, gradient: d3.Selection<SVGElement>, colors: string[], offSetValue: number[]) {
            if (this.settings.fillOption === `value`) {
                let context: this = this;
                range.style("fill", (): string => {
                    gradient = context.svgLinear.append("svg:linearGradient");
                    ...
                    if (this.settings.colorOption === `solidcolor`) {
                        gradient.append('stop').attr('offset', '0%')
                            .attr('stop-color', context.getDarkShade(colors[4], 0.5)).attr('stop-opacity', 1);
                        gradient.append('stop').attr('offset', `${offSetValue[4]}%`)
                            .attr('stop-color', colors[4]).attr('stop-opacity', 1);
                        gradient.append('stop').attr('offset', `${offSetValue[4]}%`)
                        ...
                        gradient.append('stop').attr('offset', `${offSetValue[1]}%`)
                            .attr('stop-color', context.getDarkShade(this.settings.ComparisonFillColor, 0.5)).attr('stop-opacity', 1);
        ...
        private targetline(hMarker: number, marker_select, xScale: d3.scale.Linear<number, number>, updateVar: any,
            ...
            if (this.settings.showRange) {
                ...
                if (this.settings.rangeStyle === "dotted") { marker_min_max.style("stroke-dasharray", ("1,1")); } else if (this.settings.rangeStyle === "dashed") {
                    marker_min_max
        ...
        private verticalshowcolor(colors: string[], offSetValue: number[], gradient: d3.Selection<SVGElement>, range: d3.selection.Update<number>
            ...
            if (this.settings.fillOption === `value`) {
                this.fill(gradient, range, offSetValue);
            } else {
                if (this.settings.colorOption === `solidcolor`) {
                    let context: this = this;
                    range.style("fill", (d: number, index: number): string => {
                        if (index === 0) {
                            ...
                            if (index === 1) {
                                gradient.append("stop").attr("offset", "0%")
                                    .attr("stop-color",
                                        context.getDarkShade(context.colorsGlobal[4 - index], 0.5)).attr("stop-opacity", 1);
                            } else {
        ...
        private verticalbody(range: d3.selection.Update<number>, modHeight: number, yScale: d3.scale.Linear<number, number>, leftPos: number,
            ...
            measure2: d3.selection.Update<number>, sortedMeasure2: number[]
            , legendColor: string[], vDataLabel: d3.Selection<SVGElement>, actualVal: string, svgheight: number, actualTooltip: string, percentageVal: PrimitiveValue): d3.Selection<SVGElement> {
            ...
            if (categoryFlag === 0) {
                measure = this.svgLinear
                    .append(`rect`).classed(`measure`, true).style(`fill`, "url(#gradient)").attr(`transform`, "rotate(180)");
                measure
                    .attr(`width`, measureWidth).attr(`x`, measureLeftPos).attr(`y`, -yScale(yScale.domain()[0]));
                if (this.settings.animationToggle) {
                    measure.attr(`height`, 0)
                        .transition().duration(this.settings.animationTime * 1000)
                        .attr(`height`, yScale(yScale.domain()[0]) - yScale(this.data.actual));
                } else { measure.attr(`height`, yScale(yScale.domain()[0]) - yScale(this.data.actual)); }
            } else {
                measure2 = this.svgLinear.selectAll(`rect.measure`)
                    .data(sortedMeasure2);
                measure2.enter().append(`rect`).attr("class", "measure").attr(`id`, (d: number, index: number): string => { return `measureId${index}`; });
                ...
                measure2.attr(`width`, (d: number, index: number): number => { return measureWidth / (index / legendLength + 1); })
                    .attr(`x`, (d: number, index: number): number => { return ((1.085 * measureLeftPos - (measureWidth / (index / legendLength + 1))) / 2); })
                    .attr(`y`, -yScale(yScale.domain()[0]))
                    .attr(`transform`, "rotate(180)");
                if (this.settings.animationToggle) {
                    measure2.attr(`height`, (d: number): number => { return 0; })
                        .transition().duration(this.settings.animationTime * 1000)
                        .attr(`height`, (d: number): number => { return (yScale(yScale.domain()[0]) - yScale(d)) < 0 ? 0 : (yScale(yScale.domain()[0]) - yScale(d)); });
                } else { measure2.attr(`height`, (d: number): number => { return (yScale(yScale.domain()[0]) - yScale(d)) < 0 ? 0 : (yScale(yScale.domain()[0]) - yScale(d)); }); }
            }
    ...
    ${(modHeight - adjustFactor * difference)}${`,`}${(svgheight - 20)} )`)
            ...
            } else { $(".data_percentagev").hide(); }
            return measure;
        }
        private legendpos(legend_tab_selectAll, viewport: IViewport, legend, modHeight: number, divWidth: number, legendHeight: IViewport, xScale: d3.scale.Linear<number, number>) {
            ...
            if (this.data.minFlag) {
                ...
                if (this.data.min && this.data.minFlag) {
                    ...
                    if (this.isInteger(Number(minToolTipData))) {
                        ...
                        tooltipData.push({
                            displayName: this.data.minColName, value: Number(minTooltipVal).toFixed(2)
                        });
                    }
                }
            }
        ...
        private cursorposition(legendPosSettings: any, legendHeight: IViewport, rightMargin: number) {
            ...
            let imagetab_selectAll = d3.selectAll(".lg_imagetab"); let data_Tab_selectAll = d3.selectAll(".lg_data_tab");
            ...
            } else { imagetab_selectAll.style("margin-top", `10px`); }
            if (this.data.states.length === 0) {
                if (categoryFlag === 0 && this.settings.showColor) {
                    this.data.states.push(this.data.max); this.data.states.push(this.settings.range4);
                    this.data.states.push(this.settings.range3); this.data.states.push(this.settings.range2);
                    this.data.states.push(this.settings.range1);
                } else { this.data.states.push(this.data.max); }
            }
        }
        private markerline(markerLine, marker_select, vMarker, markerTriangle_select, yScale, modHeight, measureLeftPos, vMarkerMin, vMarkerMax, minRangeValue, maxRangeValue) {
            markerLine.on("mouseover", (): void => {
                marker_select.attr("stroke-width", "5");
            });
            markerLine.on("mouseout", (): void => {
            ...
            if (this.settings.showRange) {
                ...
                } else if (this.settings.rangeStyle === "dashed") { marker_min_max.style("stroke-dasharray", ("3, 3")); } else { marker_min_max.style("stroke-dasharray", ("")); }
            }
        }
        private abovemarker(options: VisualUpdateOptions, updatedText: string, availablewidth: number,
            ...
            } else { this.targetLegend.selectAll("*").remove(); this.bestLegend.selectAll("*").remove(); }
        }
        private colors(updateVar, viewport, legendHeight, width, halfMaxFormattedDataWidth, percentageVal): PrimitiveValue {
            ...
            if (this.settings.Orientation === `Horizontal`) {
                ...
                }).style("margin-left", `${(viewport.width / 2) - (updateVar.modHeight / 2)}px`);
            ...
            if (this.data.target) {
                if (!this.settings.showRemainingPercentage) {
                    if (this.data.actual < 0 && this.data.target < 0) {
                        ...
                        percentageVal = (this.data.actual * percentVal) / this.data.target; percentageVal = parseFloat(Number(percentageVal).toFixed(2));
                    }
                } else {
                    if (this.data.actual < 0 && this.data.target < 0) {
                        ...
                        percentageVal = (this.data.actual * percentVal) / this.data.target; percentageVal = percentVal - percentageVal < 0 ? 0 : percentVal - percentageVal;
                        percentageVal = parseFloat(Number(percentageVal).toFixed(2));
                    }
                }
            } else { percentageVal = percentVal; }
        ...
        private margincolor(updateVar1) {
            if (this.settings.fillOption === `value`) {
                ...
                if (this.settings.percentageVal1 > 100) {
                    ...
                    if (this.settings.percentageVal2 > (100 - this.settings.percentageVal1)) {
                        this.settings.percentageVal2 = 100 - this.settings.percentageVal1; this.settings.percentageVal3 = 0;
                    } else {
        ...
        private colorshow(updateVar1, sortedMeasure, sortedRanges, range, colors, offSetValue, gradient, updateVar): d3.Selection<SVGElement> {
            for (let iterator: number = 0; iterator < actualValue.length; iterator++) { sortedMeasure[iterator] = actualValue[actualValue.length - iterator - 1]; }
            if (categoryFlag === 0 && this.settings.showColor) {
                this.data.states.push(this.data.max);
                if (this.settings.fillOption === `value`) { this.data.states.push(updateVar1.margin4); }
            ...
            if (categoryFlag === 0 && this.settings.showColor) {
                ...
                this.colorfill(range, gradient, colors, offSetValue);
                updateVar1.measureHeight = updateVar.modHeight / 2;
                updateVar1.measureYPos = updateVar.modHeight / 4;
            } else {
                gradient = this.svgLinear.append("svg:linearGradient");
        ...
        private height(legend_tab_selectAll: any, verticalCacheHeight: number, horizontalCacheHeight: number, legendHeight: IViewport, viewport: IViewport, yScale: d3.scale.Linear<number, number>) {
            if (categoryFlag === 0 && this.settings.Orientation === "Horizontal") {
                if (this.settings.legendNewPosition === "aboveMarker") { legend_tab_selectAll.style("margin-top", `${((viewport.height / 2) - horizontalCacheHeight)}px`); }
                ...
                if (this.settings.legendPos === "Top" || this.settings.legendPos === "Top center") {
                    ...
                    } else { legend_tab_selectAll.style("margin-top", `${legendHeight.height}px`); }
                } else {
                    if (this.settings.legendNewPosition === "aboveMarker") {
                        legend_tab_selectAll
                            .style("margin-top", `${((viewport.height / 2) - horizontalCacheHeight)}px`);
                ...
                if (this.settings.legendPos === "Top" || this.settings.legendPos === "Top center") {
                    ...
                    } else { legend_tab_selectAll.style("margin-top", `${(1 + legendHeight.height)}px`); }
                } else {
                    if (this.settings.legendNewPosition === "aboveMarker") {
                        legend_tab_selectAll
                            .style("margin-top", `${yScale(this.data.target) - verticalCacheHeight}px`);
        ...
        private view(options: VisualUpdateOptions): IViewport {
            ...
            if ((this.settings.Orientation === `Horizontal`) && (this.settings.legendPos === "Top" || this.settings.legendPos === "Top center")) {
                if ((viewport.height / 2) - this.legend.getMargins().height < trendLabelHeight + targetLabelHeight) {
                    if (!trendLabelFlag) { showTrendStatus = this.settings.showTrend; trendLabelFlag = true; if (this.settings.legendNewPosition === "topLeft") { $(".lg_legend_tab").hide(); } }
                    this.settings.showTrend = false;
                } else { if (trendLabelFlag) { this.settings.showTrend = showTrendStatus; trendLabelFlag = false; $(".lg_legend_tab").show(); } }
        ...
        private legendspace(legendPosSettings: any, legendHeight: IViewport, viewport: IViewport) {
            ...
            } else { this.settings.showColor = getValue<number>(this.dataView.metadata.objects, `colorSelector`, `show`, 0) <= 0 ? false : getValue<number>(this.dataView.metadata.objects, `colorSelector`, `show`, 0); }
            if (this.settings.animationTime > 6) { this.settings.animationTime = 6; } else if (this.settings.animationTime < 1) { this.settings.animationTime = 1; }
            ...
            if (categoryFlag === 1 && this.settings.Orientation === `Vertical` &&
                ...
                (legendPosSettings === "Bottom" || legendPosSettings === "Bottom center")) { viewport.height = viewport.height - legendHeight.height; }
        }
        private verticalcolor(range: d3.selection.Update<number>, gradient: d3.Selection<SVGElement>) {
            gradient = this.svgLinear.append("svg:linearGradient"); gradient.attr("id", "gradient2").classed("gradientSVG", true).attr("x1", "100%").attr("y1", "100%")
                .attr("x2", "0%")
                .attr("y2", "100%")
                .attr("spreadMethod", "pad");
        ...
        private horizontalcolor(gradient: d3.Selection<SVGElement>, range: d3.selection.Update<number>) {
            gradient = this.svgLinear.append("svg:linearGradient");
            gradient.attr("id", "gradient2").classed("gradientSVG", true).attr("x1", "100%").attr("y1", "0%").attr("x2", "100%").attr("y2", "100%").attr("spreadMethod", "pad");
        ...
        private gradient(measureColor: string, gradient: d3.Selection<SVGElement>) {
            gradient = this.svgLinear.append("svg:linearGradient");
            gradient.attr("id", "gradient").classed("gradientSVG", true).attr("x1", "100%").attr("y1", "0%").attr("x2", "100%").attr("y2", "100%").attr("spreadMethod", "pad");
        ...
        private category(options: VisualUpdateOptions, updatedText: string, legend_tab_selectAll: any, viewport: IViewport, updateVar: any, legendHeight: IViewport, trend1Val: string, upArrow: string, percentageFontTrend: number
            , trend2Val: string, className: string, translateVal: string
            , axisFunction: d3.svg.Axis, $this: this, axisFormatter: IValueFormatter, halfMaxFormattedDataWidth: number) {
            ...
            this.showtrend(trend1Val, options, availablewidth, upArrow, percentageFontTrend, trend2Val, className, translateVal, axisFunction, updateVar.height, $this, axisFormatter, halfMaxFormattedDataWidth, viewport)
            if (this.data.target < this.data.min || !(this.data.targetSet) || this.data.target > this.data.max) { this.svgLinear.selectAll(`line.marker`).remove(); this.svgLinear.selectAll(`line.markerTilt`).remove(); } else { this.svgLinear.selectAll(`.marker`).style(`display`, `block`); this.svgLinear.selectAll(`line.markerTilt`).style(`display`, `block`); }
            if (categoryFlag === 1) { prevFlag = options.dataViews[0].categorical.categories[0].values.length; }
            if (categoryFlag === 1 && this.settings.Orientation === `Vertical` &&
                (this.settings.legendPos === "Bottom" || this.settings.legendPos === "Bottom center" || this.settings.legendPos === "Top" || this.settings.legendPos === "Top center")) { viewport.height = viewport.height + legendHeight.height; } else if (categoryFlag === 1 && this.settings.Orientation === `Horizontal` &&
                    (this.settings.legendPos === "Right" || this.settings.legendPos === "Right center" ||
                        this.settings.legendPos === "Left" || this.settings.legendPos === "Left center")) { viewport.width = viewport.width + legendHeight.width + 15; }
            else if (categoryFlag === 1 && this.settings.Orientation === `Horizontal` &&
                (this.settings.legendPos === "Bottom" || this.settings.legendPos === "Bottom center")) { viewport.height = viewport.height + legendHeight.height; }
            ...
            let tooltipData: ITooltipData[] = []; let minToolTipData: number; let targetToolTipData: number; let actualToolTipData: number = this.data.actual; let maxToolTipData: number;
        ...
        private scale() {
            ...
            this.svgLinear.selectAll(`rect.measure`).remove();
            $(".lg_data_total").hide();
            if (this.settings.showlabel) { $(".lg_data_total").show(); }
            if (this.settings.showPercentage) { $(".lg_data_percentage").show(); } else { $(".lg_data_percentage").hide(); }
            this.trendValue1.style(`text-align`, `right`); this.trendValue2.style(`text-align`, `right`);
            this.data.min = this.xScale.domain()[0]; this.data.max = this.xScale.domain()[1];
        }
        public update(options: VisualUpdateOptions): void {
            try {
                this.events.renderingStarted(options); let $this: this = this; $this.visualOptions = options; let vMarker: number; categoryLegend = []; let gradient: d3.Selection<SVGElement>;
                this.removeAllClasses(); this.svg.style("margin-left", 0); this.svg.style("margin-top", 0); this.dataView = options.dataViews[0];
                if (!options.dataViews || !this.dataView || !this.dataView.metadata) { return; }
                this.data = this.converter(this.dataView, options); let viewport = this.view(options)
                if (!this.data.actualExists) { const message: string = 'Please add "Actual value" field'; this.rootElement.append("div").classed("lg_ErrorMessage", true).text(message).attr("title", message); return; } let legendHeight: IViewport = this.legend.getMargins();
                let legendPosSettings = this.settings.legendPos;
                if (this.data.actual === null) { return; }
                this.getSettings(this.dataView.metadata.objects); this.legendspace(legendPosSettings, legendHeight, viewport); let rightMargin: number; d3.selectAll(".legendItem").style("cursor", "pointer");
                this.cursorposition(legendPosSettings, legendHeight, rightMargin); let legendColor: string[] = []; let sortedRanges: number[] = this.data.states; let percentageVal: PrimitiveValue;
                let actualVal: string; let trend1Val: string; let trend2Val: string, width: number, updateVar = { height: 0, modHeight: 0 }; const svgheight: number = viewport.height;
                if (individualFlag !== 0) { legendColor[0] = categoryLegend[individualFlag - 1].color; } else {
                    let colorIndex: number = 0;
                    for (let index: number = 0; index < categoryLegend.length; index++) { if (tempSelectionId !== null && categoryLegend[index].key === selectedKey) { legendColor[colorIndex] = categoryLegend[index].color; colorIndex++; } else { legendColor[index] = categoryLegend[categoryLegend.length - index - 1].color; } }
                }
                if (this.data.max === null) { this.data.max = 0; }
                let axisFormatter: IValueFormatter = valueFormatter.create({ format: this.data.actualFormat, precision: this.settings.scaleDecimalPlaces, value: this.settings.scaleDisplayUnits === 0 ? this.getValueUpdated(this.data.max) : this.settings.scaleDisplayUnits });
                let textProperties: TextProperties = { fontFamily: `${this.settings.scaleFontFamily}`, fontSize: `${this.settings.scaleFontSize}px`, text: axisFormatter.format(this.data.max) }, maxFormattedDataWidth: number = textMeasurementService.measureSvgTextWidth(textProperties), halfMaxFormattedDataWidth: number = maxFormattedDataWidth / 2;
                percentageVal = this.colors(updateVar, viewport, legendHeight, width, halfMaxFormattedDataWidth, percentageVal);
                let minRangeValue: number, maxRangeValue: number; let vDataLabel: d3.Selection<SVGElement>; let upArrow: string = `&#8599`; let percentageFont: number = this.settings.fontSize / 2.0; let percentageFontTrend: number = this.settings.trendfontSize / 2.5;
                this.xScale = d3.scale.linear().domain([this.data.min, this.data.max]).range([30, viewport.width]).nice();
                let yScale: d3.scale.Linear<number, number> = d3.scale.linear().domain([this.data.min, this.data.max]).range([viewport.height - 15, 15]).nice(), xAxis: d3.svg.Axis; const precisionValue: number = this.settings.markerWidth; const precisionValueTrend: number = this.settings.lineWidth;
                actualVal = this.getFormattedData(this.data.actual, this.settings.labelDisplayUnits, precisionValue, this.data.actualFormat); trend1Val = this.getFormattedData(this.data.trendValueOne, this.settings.trendDisplayUnits, precisionValueTrend, this.data.trend1Format);
                ...
                const textProps: TextProperties = { fontFamily: this.settings.fontFamily, fontSize: `${this.settings.fontSize}px`, text: actualVal }, actualTooltip: string = this.getFormattedTooltipData(this.data.actualFormat, this.data.actual)
                const horizontalWidth: number = (this.settings.showPercentage) ? (options.viewport.width / 2.1) - 20 : options.viewport.width - 30; let updatedText: string = textMeasurementService.getTailoredTextOrDefault(textProps, horizontalWidth);
                this.actual.text(updatedText).attr("title", actualTooltip).style(`font-size`, `${this.settings.fontSize}px`).style(`font-family`, this.settings.fontFamily).style(`color`, this.settings.DataColor).style(`margin-right`, `${percentageFont}px`);
                const textPropspercent: TextProperties = { fontFamily: this.settings.percentagefontFamily, fontSize: `${this.settings.percentagefontSize}px`, text: `${percentageVal}%` }, dataWidth: number = $(".lg_data_total").width(); const viewportWidthCache: number = 70, updatedTextpercent: string = textMeasurementService.getTailoredTextOrDefault(textPropspercent, options.viewport.width - dataWidth - viewportWidthCache);
                this.percentage.text(updatedTextpercent).attr("title", `${percentageVal}%`).style(`font-size`, `${this.settings.percentagefontSize}px`).style(`color`, this.settings.PercentageDataColor).style(`font-Family`, this.settings.percentagefontFamily);
                let actualFormatter: IValueFormatter = valueFormatter.create({ format: this.data.actualFormat }), className: string, translateVal: string, axisFunction: d3.svg.Axis; let legend_tab_selectAll = d3.selectAll(".lg_legend_tab");
                const horizontalCacheHeight: number = 38, verticalCacheHeight: number = 13; this.height(legend_tab_selectAll, verticalCacheHeight, horizontalCacheHeight, legendHeight, viewport, yScale)
                if (this.settings.Orientation === `Horizontal`) {
                    this.scale(); let xTextProperties: TextProperties = { fontFamily: `${this.settings.scaleFontFamily}`, fontSize: `${this.settings.scaleFontSize}px`, text: axisFormatter.format(this.data.max) }; let xMaxFormattedDataWidth: number = textMeasurementService.measureSvgTextWidth(xTextProperties); let xHalfMaxFormattedDataWidth: number = maxFormattedDataWidth / 2;
                    const distance: number = (xHalfMaxFormattedDataWidth * 10 / xHalfMaxFormattedDataWidth); this.xScale.range([distance, viewport.width - distance]); xAxis = d3.svg.axis().scale(this.xScale).tickFormat(actualFormatter.format).orient("down");
                    minRangeValue = (this.settings.MinRangeValue === null || this.settings.MinRangeValue < this.data.min || this.settings.MinRangeValue > this.data.max) ? this.data.min : this.settings.MinRangeValue; maxRangeValue = (this.settings.MaxRangeValue === null || this.settings.MaxRangeValue < this.data.min || this.settings.MaxRangeValue > this.data.max) ? this.data.max : this.settings.MaxRangeValue;
                }
                let colors: string[] = []; let offSetValue: number[] = [], measureColor: string = this.settings.ActualFillColor; this.removeSvgMarker();
                if (this.data.target !== null) { if (this.data.actual > this.data.target) { measureColor = this.settings.ActualFillColorGreater; } else if (this.data.actual === this.data.target) { measureColor = this.settings.ActualFillColorEqual; } }
                if (this.settings.Orientation === `Horizontal`) {
                    let updateVar1 = { margin1: 0, margin2: 0, margin3: 0, margin4: 0, valueToPercent1: 0, valueToPercent2: 0, valueToPercent3: 0, valueToPercent4: 0, measureHeight: 0, measureYPos: 0 }
                    this.data.max = this.xScale.domain()[1]; this.data.states = []; let sortedMeasure: number[] = []; this.margincolor(updateVar1);
                    for (let iterator: number = 0; iterator < actualValue.length; iterator++) { sortedMeasure[iterator] = actualValue[actualValue.length - iterator - 1]; }
                    if (categoryFlag === 0 && this.settings.showColor) { this.data.states.push(this.data.max); if (this.settings.fillOption === `value`) { this.data.states.push(updateVar1.margin4); } this.data.states.push(updateVar1.margin3); this.data.states.push(updateVar1.margin2); this.data.states.push(updateVar1.margin1); }
                    else { this.data.states.push(this.data.max); }
                    sortedRanges = this.data.states;
                    if (this.settings.fillOption === `value`) { this.range = this.svgLinear.append(`rect`).classed(`rectRange`, true).data(sortedRanges); this.range.enter().append("rect"); }
                    else { if (this.settings.Orientation === `Horizontal`) { this.range = this.svgLinear.append(`rect`).classed(`rectRange`, true).data(sortedRanges); this.range.enter().append("rect"); } }
                    if (categoryFlag === 0 && this.settings.showColor) {
                        const zoneLen: number = this.colorsGlobal.length; for (let index: number = 0; index < zoneLen; index++) { colors.push(this.colorsGlobal[index]); offSetValue.push(this.data.states[index]); }
                        this.colorfill(this.range, gradient, colors, offSetValue); updateVar1.measureHeight = updateVar.modHeight / 2; updateVar1.measureYPos = updateVar.modHeight / 4;
                    } else { this.horizontalcolor(gradient, this.range); updateVar1.measureHeight = updateVar.modHeight; updateVar1.measureYPos = 0; }
                    this.range.attr(`x`, this.xScale(this.xScale.domain()[0])).attr(`width`, (d: number): number => { return (this.xScale(d) - this.xScale(this.xScale.domain()[0])) < 0 ? 0 : (this.xScale(d) - this.xScale(this.xScale.domain()[0])); }).attr(`height`, updateVar.modHeight);
                    this.gradient(measureColor, gradient);
                    this.measure = this.horizontalanimation(this.measure, updateVar1.measureHeight, this.xScale, updateVar1.measureYPos, this.measure2, sortedMeasure, updateVar, gradient, legendColor);
                    if (this.data.max <= this.data.min) { if (this.measure === undefined) { $(".rectRange").remove(); $(".measure").remove(); this.nullValueError(); } this.measure.style(`display`, `none`); }
                    let hMarker: number = this.xScale(this.data.target), hMarkerMin: number = this.xScale(minRangeValue), hMarkerMax: number = this.xScale(maxRangeValue), marker_select = d3.select(".marker");
                    let markerTriangle_select = d3.select(".markerTriangle"), markerLine: d3.Selection<SVGElement> = this.svgLinear.append(`line`).classed(`marker`, true).style(`stroke`, `brown`).attr({ x1: hMarker, x2: hMarker, y1: 0, y2: updateVar.modHeight });
                    markerLine.on("mouseover", (): void => { marker_select.attr("stroke-width", "5"); }); markerLine.on("mouseout", (): void => { marker_select.attr("stroke-width", "1"); });
                    ...
                    translateVal = `translate(0,${(updateVar.modHeight + 15)} )`; axisFunction = xAxis; className = "lg_xLabels";
                }
                else {
                    offSetValue = []; let margin1: number, margin2: number, margin3: number, margin4: number, valueToPercent1: number, valueToPercent2: number; let valueToPercent3: number; let valueToPercent4: number; this.data.min = yScale.domain()[0]; this.data.max = yScale.domain()[1];
                    minRangeValue = (this.settings.MinRangeValue === null || this.settings.MinRangeValue < this.data.min || this.settings.MinRangeValue > this.data.max) ? this.data.min : this.settings.MinRangeValue; maxRangeValue = (this.settings.MaxRangeValue === null || this.settings.MaxRangeValue < this.data.min || this.settings.MaxRangeValue > this.data.max) ? this.data.max : this.settings.MaxRangeValue;
                    let yAxis: d3.svg.Axis = d3.svg.axis().scale(yScale).tickFormat(actualFormatter.format).orient("right");
                    const leftPos: number = -(viewport.height / 11); let measureLeftPos: number; let measureWidth: number; let comparsionWidth: number; this.svgLinear.selectAll(`rect.range`).remove(); this.data.max = yScale.domain()[1]; this.data.states = [];
                    if (this.settings.fillOption === `value`) {
                        const range12Max: number = Math.max(this.settings.range1, this.settings.range2), range123Max: number = Math.max(this.settings.range1, this.settings.range2, this.settings.range3);
                        this.settings.range1 = this.settings.range1 === null ? null : this.settings.range1 < this.data.min ? null : this.settings.range1 > this.data.max ? null : this.settings.range1, this.settings.range2 = this.settings.range2 === null ? null : this.settings.range2 < this.settings.range1 ? this.settings.range1 : this.settings.range2 > this.data.max ? null : this.settings.range2;
                        this.settings.range3 = this.settings.range3 === null ? null : this.settings.range3 < range12Max ? range12Max : this.settings.range3 > this.data.max ? null : this.settings.range3, this.settings.range4 = this.settings.range4 === null ? null : this.settings.range4 < range123Max ? range123Max : this.settings.range4 > this.data.max ? null : this.settings.range4;
                        valueToPercent1 = (this.settings.range1 - this.data.min) / (this.data.max - this.data.min), valueToPercent2 = (this.settings.range2 - this.data.min) / (this.data.max - this.data.min), valueToPercent3 = (this.settings.range3 - this.data.min) / (this.data.max - this.data.min), valueToPercent4 = (this.settings.range4 - this.data.min) / (this.data.max - this.data.min), margin1 = valueToPercent1 * 100, margin2 = valueToPercent2 * 100, margin3 = valueToPercent3 * 100, margin4 = valueToPercent4 * 100;
                    } else {
                        if (this.settings.percentageVal1 > 100) { this.settings.percentageVal1 = 100, this.settings.percentageVal2 = 0, this.settings.percentageVal3 = 0; }
                        else {
                            if (this.settings.percentageVal2 > (100 - this.settings.percentageVal1)) { this.settings.percentageVal2 = 100 - this.settings.percentageVal1, this.settings.percentageVal3 = 0; }
                            else { if (this.settings.percentageVal3 > (100 - this.settings.percentageVal1 - this.settings.percentageVal2)) { this.settings.percentageVal3 = 100 - this.settings.percentageVal1 - this.settings.percentageVal2; } }
                        } margin1 = this.settings.percentageVal1, margin2 = margin1 + this.settings.percentageVal2, margin3 = margin2 + this.settings.percentageVal3;
                    }
                    if (categoryFlag === 0 && this.settings.showColor) { this.data.states.push(this.data.max); if (this.settings.fillOption === `value`) { this.data.states.push(margin4); } this.data.states.push(margin3); this.data.states.push(margin2); this.data.states.push(margin1); }
                    else { this.data.states.push(this.data.max); }
                    sortedRanges = this.data.states; let sortedMeasure2: number[] = [];
                    for (let index: number = 0; index < actualValue.length; index++) { sortedMeasure2[index] = actualValue[actualValue.length - index - 1]; }
                    if (this.settings.fillOption === "value") { this.range = this.svgLinear.append(`rect`).classed(`rectRange`, true).data(sortedRanges); this.range.enter().append("rect"); } else { this.range = this.svgLinear.append(`rect`).classed(`rectRange`, true).data(sortedRanges); this.range.enter().append("rect"); }
                    if (this.settings.showColor) {
                        ...
                        measureLeftPos = leftPos + updateVar.modHeight / 4, measureWidth = updateVar.modHeight / 2, comparsionWidth = updateVar.modHeight / 6;
                    } else { this.verticalcolor(this.range, gradient); measureLeftPos = leftPos; measureWidth = updateVar.modHeight; comparsionWidth = updateVar.modHeight / 3; }
                    this.measure = this.verticalbody(this.range, updateVar.modHeight, yScale, leftPos, gradient, measureColor, this.measure, measureWidth, measureLeftPos, this.measure2, sortedMeasure2, legendColor, vDataLabel, actualVal, svgheight, actualTooltip, percentageVal)
                    if (this.data.max <= this.data.min) { if (this.measure === undefined) { $(".rectRange").remove(); $(".measure").remove(); this.nullValueError(); } this.measure.style(`display`, `none`); }
                    const eleLabel: JQuery = $(".LG_verticalDataLabel"); const eleTrend: JQuery = $(".lg_imagetab");
                    if (eleLabel && eleLabel.length && eleLabel[0]) { const domPosition: ClientRect = eleLabel[0].getBoundingClientRect(); if (domPosition.left < 0) { $(".LG_verticalDataLabel text").remove(); } }
                    let vMarkerMin: number = yScale(minRangeValue); let vMarkerMax: number = yScale(maxRangeValue); vMarker = yScale(this.data.target); let marker_select = d3.select(".marker"), markerTriangle_select = d3.select(".markerTriangle"); let markerLine: d3.Selection<SVGElement> = this.svgLinear.append(`line`).classed(`marker`, true).style(`stroke`, `brown`).attr({ x1: 3, x2: updateVar.modHeight + 3, y1: vMarker, y2: vMarker });
                    this.markerline(markerLine, marker_select, vMarker, markerTriangle_select, yScale, updateVar.modHeight, measureLeftPos, vMarkerMin, vMarkerMax, minRangeValue, maxRangeValue);
                    translateVal = `${`translate(`}${(updateVar.modHeight + 15)}${`,`}${(0)})`; axisFunction = yAxis; className = "lg_yLabels";
                }
                this.category(options, updatedText, legend_tab_selectAll, viewport, updateVar, legendHeight, trend1Val, upArrow, percentageFontTrend, trend2Val, className, translateVal, axisFunction, $this, axisFormatter, halfMaxFormattedDataWidth)
        ...
        private colorselector(objectEnumeration: VisualObjectInstance[], objectName: string) {
            if (this.settings.fillOption === `value`) {
        ...
        private trendlabels(objectEnumeration: VisualObjectInstance[], objectName: string) {
        ...
        public enumerateObjectInstances(options: EnumerateVisualObjectInstancesOptions):
            ...
            let objectName: string = options.objectName; let objectEnumeration: VisualObjectInstance[] = [];
            ...
            switch (options.objectName) {
                ...
                case `legendSettings`:
                    objectEnumeration.push({
                        objectName, selector: null, properties: {
                            show: this.settings.legendShow, position: this.settings.legendNewPosition, fill: this.settings.legendColor, fontSize: this.settings.legendFontSize, fontFamily: this.settings.legendFontFamily,
                            displayUnits: this.settings.legendDisplayUnits, decimalPlaces: this.settings.legendDecimalPlaces
                        }
                    });
                    break;
                case "categorySettings":
                    if (categoryFlag === 1) {
                        objectEnumeration.push({ objectName, selector: null, properties: { fontSize: this.settings.categoryFontSize, title: this.settings.categoryTitle, color: this.settings.legendTextColor, position: this.settings.legendPos } });
                    }
                    break;
                case "colors":
                    if (categoryFlag === 1) {
                        for (const index of categoryLegend) { objectEnumeration.push({ objectName, displayName: index.key, properties: { fillColor: { solid: { color: index.color } } }, selector: index.selectionId.getSelector() }); }
                    }
                    break;
                case "animationEffect":
                    ...
                    break;
                case `colorSelector`:
                    if (categoryFlag === 0) { this.colorselector(objectEnumeration, objectName); }
                    break;
                case `ScaleSettings`:
                    objectEnumeration.push({
                        objectName,
                ...
                case `Indicator`:
                    if (this.data.trendValueOne || this.data.trendValueTwo) {
                        let props: {} = {};
                        if (this.data.trendValueOne) { props[`Indicator1`] = this.settings.Indicator1; }
                        if (this.data.trendValueTwo) { props[`Indicator2`] = this.settings.Indicator2; }
                        objectEnumeration.push({ objectName, selector: null, properties: props });
                    }
                    break;
                case `general`:
                    objectEnumeration.push({
                        ...
                        properties: {
                            ...
                            ActualFillColorGreater: { solid: { color: this.settings.ActualFillColorGreater } }, ComparisonFillColor: { solid: { color: this.settings.ComparisonFillColor } }
                        }
                    });
                    break;
                case `labels`:
                    ...
                    break;
                case `trendLabels`:
                    if (this.data.trendValueOne || this.data.trendValueTwo) { this.trendlabels(objectEnumeration, objectName); }
                    break;
